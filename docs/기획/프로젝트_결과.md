...
| 항목 | 내용 |
|---|---|
| 과정명 | SKN29 3기 — 4팀 최종 프로젝트 |
| 제출일 | 2026-06-17 |
| 핵심 스택 | LangGraph · ChromaDB · Neo4j · GPT-5.4-mini · Qwen3.5-4B |
...
flowchart TD
    ...
    R -->|RAG 검색| A[internal_rag]
    R -->|그래프 탐색| B[graph_db]
    R -->|수리 계산| C[sql_db]
    R -->|법령·외부 API| D[external_api]
    R -->|조건 불명확| E[clarify]
    R -->|바로 생성 가능| F[generate]
    A & B & C & D --> F
    F --> V[verify]
    V -->|조건 누락 — 재시도 최대 5회| R
    V -->|검증 완료| END([END])
...
아래는 원천 자료부터 운영 저장소까지의 데이터 흐름이다.
...
flowchart LR
    ...
    subgraph "data/processed/"
        J1["hanja_documents.json\n2,420건"]
        J2["suri_documents.json\n81건"]
        J3["law_articles.json\n250건"]
        J4["urimalsam_names.json\n301건"]
        J5["paper_documents.json\n264건"]
        J6["ohaeng_documents.json\n125건"]
    end
    subgraph 운영 저장소
        C["ChromaDB\n6개 컬렉션 / 3,457건"]
        N["Neo4j\n한자·오행 그래프"]
...
| 평가 케이스 수 | 11 |
| 성공 처리 | 11 / 11 |
| Context Relevance (컨텍스트 관련성) | 4.55 |
| Groundedness (근거성) | 3.82 |
| Answer Relevance (답변 관련성) | 3.91 |
| 전체 평균 | 4.09 |
...
> **평가 지표 비고**: GPT Pipeline은 CR (Context Relevance) · GR · AR 기준으로, Qwen은 FA (Factual Accuracy) · GR · AR 기준으로 평가되었다. CR과 FA는 측정 관점이 다르므로 직접 비교는 종합 평균 기준으로 해석한다.

아래는 각 시스템의 지표별 점수 비교이다.

```mermaid
xychart-beta
    title "GPT Pipeline vs Qwen 파인튜닝 — 지표별 점수 (5점 만점)"

...
소형 로컬 모델(Qwen 3.5 4B)의 텍스트 파싱 오류를 방지하기 위해, 모든 툴은 단순 문자열이 아닌 **구조화된 JSON 문자열**을 반환합니다.
...
이 문서는 6~7단계 에이전트 확장 작업을 위한 가이드로 활용됩니다. LangGraph 통합 시, 반드시 커스텀 ToolNode를 구현하여 본 서버가 반환하는 JSON을 파싱(데이터 분리)해야 합니다.

...
> 문서 상태: 한자 유니코드/오행 최종 정제 기록. 현재 산출물은 운영 한자 2,420건의 기준 데이터다.

| 항목 | 현재 기준 |
|---|---|
| 최종 JSON | `hanja_unicode_ohaeng_verified_corrected.json` 2,420건 |
| 최종 CSV | `hanja_unicode_ohaeng_verified_corrected.csv` 2,420건 |
| 필드별 CSV | `csv_tables/*.csv` 각 2,420건 |
| 후속 산출물 | `hanja_documents.json` 2,420건 |
| 주의 | `hanja.pdf`는 뜻/획수/오행의 완전 검증 근거라기보다 음-한자 존재 확인 보조 자료로 본다 |
...
ChromaDB, Neo4j, `db_server.py` 계산 도구 등 후속 사용처에 반영할 때는 이 파일을 직접 수정하지 않고, 별도 변환 산출물을 생성해서 사용하는 방식이 안전하다. 현재 운영 기준에서는 `hanja_documents.json`을 통해 ChromaDB와 Neo4j가 같은 한자 metadata를 바라보도록 관리하고, 수치 계산은 별도 업무 SQLite가 아니라 JSON/XLS/pandas 기반 도구로 처리한다.
